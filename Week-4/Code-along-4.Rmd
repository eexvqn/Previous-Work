...
# Enter code here
hotels %>%
filter(hotel == "Resort Hotel", children >= 1) %>%
...
hotels %>%
  ...
  select(little_ones,children,babies,hotel)
```

### 17. Creating multiple columns with mutate() (Slide #58)
```{r, eval=TRUE,echo=TRUE}
# Enter code here
which(is.na(hotels$children))
which(is.na(hotels$babies))

hotels %>%
  slice(-c(40601, 40668, 40680, 41161)) %>%
  mutate(little_ones = children + babies, 
         average_little_ones = mean(little_ones)) %>%
  select(little_ones, average_little_ones,children,babies)
```
<br>
...
hotels %>%
  ...
  select(little_ones, babies, children) %>%
